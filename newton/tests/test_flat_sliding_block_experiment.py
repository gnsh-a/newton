...
from newton.examples.contacts.experiment_flat_sliding_block import (
    SUMMARY_COLUMNS,
    SUMMARY_CSV,
    TIMESERIES_COLUMNS,
    TIMESERIES_CSV,
...
class TestFlatSlidingBlockExperiment(unittest.TestCase):
    def test_flat_sliding_block_experiment(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for row in summary_rows:
                ...
                self.assertTrue(_as_bool(row["stopped"]))
                self.assertLess(float(row["final_speed_m_per_s"]), 0.02)
                self.assertLess(abs(float(row["final_y_m"])), 1.0e-3)
                self.assertLess(float(row["final_tilt_deg"]), 0.5)
                self.assertTrue(math.isfinite(float(row["horizontal_impulse_Ns"])))
