...
###########################################################################
# Example Sliding Ridged Cube
#
# Slides a hydroelastic cube across one continuous diagonal-ridged mesh plate.
# The ridge side normals create lateral drift and yaw; contact reduction
# compresses that dense oblique contact field into a small solver set.
#
# Command: python -m newton.examples sliding_ridged_cube
#
###########################################################################
...
PLATE_HALF_EXTENT = 0.26
"""Ridged plate half-extent in X and Y [m]."""
PLATE_THICKNESS = 0.025
"""Ridged plate solid thickness below the lowest ridge [m]."""
RIDGE_AMPLITUDE = 0.009
"""Peak-to-valley ridge height [m]."""
RIDGE_PITCH = 0.055
"""Triangular-wave ridge spacing [m]."""
RIDGE_GRID_RESOLUTION = 56
"""Number of heightfield cells per plate side."""

MU_SLIDING = 1.2
"""Friction coefficient used by both hydroelastic shapes."""
KH = 1.0e9
"""Hydroelastic contact stiffness coefficient."""

SDF_MAX_RESOLUTION = 96
"""Maximum SDF grid resolution along the longest axis."""
SDF_NARROW_BAND_RANGE = (-0.01, 0.01)
"""SDF narrow-band range [m]."""
SDF_MARGIN = 0.01
"""Extra SDF AABB padding [m]."""

INITIAL_X = -0.16
"""Initial cube center X position [m]."""
INITIAL_Y = -0.03
"""Initial cube center Y position [m]."""
INITIAL_VX = 0.55
"""Initial cube X velocity [m/s]."""

RIGID_CONTACT_MAX = 65536
"""Upper bound on rigid contacts allocated by the collision pipeline."""
MUJOCO_NCONMAX = 65536
"""MuJoCo contact buffer capacity."""
MUJOCO_NJMAX = 196608
"""MuJoCo constraint Jacobian row capacity."""

OUTPUT_DIR = os.path.join("output", "sliding_ridged_cube")
"""Where CSV traces land, relative to the process cwd."""


def _triangular_wave01(value: np.ndarray) -> np.ndarray:
    phase = value - np.floor(value)
    return 1.0 - np.abs(2.0 * phase - 1.0)


def _build_ridged_plate_mesh() -> newton.Mesh:
    coords = np.linspace(-PLATE_HALF_EXTENT, PLATE_HALF_EXTENT, RIDGE_GRID_RESOLUTION + 1, dtype=np.float32)
    xx, yy = np.meshgrid(coords, coords, indexing="ij")
    ridge_coord = (xx + yy) / RIDGE_PITCH
    top_z = (RIDGE_AMPLITUDE * _triangular_wave01(ridge_coord)).astype(np.float32)
    ...
    indices: list[int] = []
    for i in range(RIDGE_GRID_RESOLUTION):
        for j in range(RIDGE_GRID_RESOLUTION):
            t00 = top_id(i, j)
            t10 = top_id(i + 1, j)
            t01 = top_id(i, j + 1)
            t11 = top_id(i + 1, j + 1)
    ...
    for j in range(RIDGE_GRID_RESOLUTION):
        ...
        t0 = top_id(RIDGE_GRID_RESOLUTION, j)
        t1 = top_id(RIDGE_GRID_RESOLUTION, j + 1)
        b0 = bottom_id(RIDGE_GRID_RESOLUTION, j)
        b1 = bottom_id(RIDGE_GRID_RESOLUTION, j + 1)
    ...
    for i in range(RIDGE_GRID_RESOLUTION):
        ...
        t0 = top_id(i, RIDGE_GRID_RESOLUTION)
        t1 = top_id(i + 1, RIDGE_GRID_RESOLUTION)
        b0 = bottom_id(i, RIDGE_GRID_RESOLUTION)
        b1 = bottom_id(i + 1, RIDGE_GRID_RESOLUTION)
        indices.extend((b0, t0, t1, b0, t1, b1))
...
class Example:
    def __init__(self, viewer, args):
        ...
        ridged_plate_mesh = _build_ridged_plate_mesh()
        builder.add_shape_mesh(
            body=-1,
            mesh=ridged_plate_mesh,
            cfg=plate_cfg,
            label="ridged_plate",
        )

        cube_z = CUBE_HALF_EXTENT + 0.45 * RIDGE_AMPLITUDE
    ...
    def test_final(self):
        ...
        assert forward_mm > 70.0, f"cube did not slide forward enough: {forward_mm:.2f} mm"
        assert np.min(zs) > 0.015, f"cube fell through the ridged plate: min z={np.min(zs):.4f} m"
        assert max(self.normal_lateral_count_gt_0_2_log) > 0, "no oblique contact normals were observed"
        assert abs(lateral_mm) > 0.5 or yaw_span > 0.5, (
            f"ridge response too small: lateral={lateral_mm:.3f} mm, yaw_span={yaw_span:.3f} deg"
        )
    ...
    def _write_csv(self) -> None:
        ...
        path = os.path.join(OUTPUT_DIR, f"sliding_ridged_cube_{suffix}.csv")
